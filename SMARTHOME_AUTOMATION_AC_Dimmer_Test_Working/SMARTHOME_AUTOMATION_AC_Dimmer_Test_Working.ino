...
const unsigned long HISTORY_SAVE_INTERVAL = 60000; // Save to history every 1 minute
...
void ICACHE_RAM_ATTR zeroCross() {
  ...
  // We need to calculate the delay from the power level (0-100)
  // Power 100% = Minimum delay
  // Power 0%   = Maximum delay (don't fire)
  // Formula: delay = (100 - power) * (HalfCycle / 100)
  
  // Calculate delay for Fan 1
  if (fan1Power > 5) { // Only fire if power is > 5%
    float delayPercent1 = (100.0 - fan1Power) / 100.0;
    float delaySeconds = (delayPercent1 * (HALF_CYCLE_MICROS - DIMMER_MIN_DELAY) / 1000000.0);
    fan1Timer.once(delaySeconds + (DIMMER_MIN_DELAY / 1000000.0), triggerFan1);
  }

  // Calculate delay for Fan 2
  if (fan2Power > 5) {
    float delayPercent2 = (100.0 - fan2Power) / 100.0;
    float delaySeconds = (delayPercent2 * (HALF_CYCLE_MICROS - DIMMER_MIN_DELAY) / 1000000.0);
    fan2Timer.once(delaySeconds + (DIMMER_MIN_DELAY / 1000000.0), triggerFan2);
  }
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topicStr == fan1_set_topic) {
    ...
    Serial.println("%");
    mqttClient.publish(fan1_status_topic, String(fan1Power).c_str(), true);
  }
  // Handle Fan 2
...
void handleIRCommand(uint32_t irCode) {
  ...
  switch (irCode) {
    ...
    case IR_SPEED_UP:
      ...
      // If both OFF, turn ON at minimum
      if (fan1Power == 0 && fan2Power == 0) {
...
void sendToFirebase(String path, String jsonData) {
  ...
  client.setBufferSizes(512, 512); // Reduce buffer size to save memory
...
void saveCurrentSensorData() {
  ...
  doc["lastUpdate"] = timeClient.getFormattedTime();  // Keep IST for display
  
  char buffer[512];
  serializeJson(doc, buffer);
  
  sendToFirebase("/sensors/current", buffer);
}
...
void savePZEMData() {
  ...
  // Send current data to Firebase (for real-time display)
  ...
  // Also publish to MQTT for real-time monitoring
  if (mqttClient.connected()) {
    ...
    mqttClient.publish(pzem_voltage_topic, String(pzemVoltage, 1).c_str(), false);
    mqttClient.publish(pzem_current_topic, String(pzemCurrent, 2).c_str(), false);
    mqttClient.publish(pzem_power_topic, String(pzemPower, 0).c_str(), false);
    mqttClient.publish(pzem_energy_topic, String(pzemEnergy, 2).c_str(), false);
    mqttClient.publish(pzem_frequency_topic, String(pzemFrequency, 1).c_str(), false);
    mqttClient.publish(pzem_pf_topic, String(pzemPowerFactor, 2).c_str(), false);
  }
}
...
void readAndPublishSensorData() {
  ...
  // Save data to Firebase (current readings)
  if (millis() - lastFirebaseUpdate >= FIREBASE_UPDATE_INTERVAL) {
    lastFirebaseUpdate = millis();
    saveCurrentSensorData();
    
    if (gasAlarmActive) {
      updateFirebaseAlarms();
    }
  }
  
  // Save to Firebase history (less frequent)
  if (millis() - lastHistorySave >= HISTORY_SAVE_INTERVAL) {
    lastHistorySave = millis();
    saveHistoricalData();
  }

  // Still publish to MQTT for real-time device control feedback
  // Create combined JSON payload for all sensors
  ...
  // Publish to MQTT (optional - for real-time monitoring)
  if (mqttClient.connected()) {
...
void readNanoSensorData() {
  ...
  Wire.requestFrom(NANO_I2C_ADDRESS, DATA_SIZE);
...
void loop() {
  ...
  // Sensors are read periodically above.
}